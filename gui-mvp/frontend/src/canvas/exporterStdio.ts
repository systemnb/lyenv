...
export async function buildManifestAndFilesStdio(
  ...
  const pluginName = (opts.pluginName || 'myplugin').trim()
  const version = (opts.version || '0.1.0').trim()
  const expose = (opts.expose && opts.expose.length ? opts.expose : [pluginName]).map(token)
  const localFile = (opts.localConfigFile || './config.yaml').trim() || './config.yaml'

  const manifest: Manifest = {
    name: pluginName,
    version,
    expose,
    config: { local_file: localFile },
    commands: [],
  }
  ...
  files.push({
    path: 'config.yaml',
    content: (opts.configYaml || '# default config for the plugin\nflow:\n  outputs: {}\n') + (opts.configYaml?.endsWith('\n') ? '' : '\n'),
  })
  ...
  // README
  const title = (opts.readmeTitle || pluginName).trim()
  const body = (opts.readmeBody || 'Generated by Canvas exporter (stdio flow).').trim()
