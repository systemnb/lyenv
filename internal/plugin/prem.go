...
// regular files are 0644, and files with shebang are 0755.
func NormalizePluginPermissions(root string) error {
	return filepath.WalkDir(root, func(path string, d fs.DirEntry, err error) error {
		...
		// Regular file
		// Default 0644, then detect shebang for executables
...
// EnsureLogsDir creates plugin logs dir if missing.
